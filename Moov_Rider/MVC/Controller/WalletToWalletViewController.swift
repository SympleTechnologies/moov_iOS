...
//  Moov_Rider
//
//  Created by Visakh on 06/09/18.
//  Copyright © 2018 Visakh. All rights reserved.
//

import UIKit

class WalletToWalletViewController: UIViewController, UITextFieldDelegate {
    ...
    func setupUI()  {
        ...
        textFieldPhoneNumber.setLeftPaddingPoints(2.0)
        ...
        AlamofireSubclass.parseLinkUsingPostMethod("wallet/transfer/user", with: params!) { (success, response, error) in
            ...
            if success == true{
                if response!["message"] as! String == "Amount mooved successfully" {
                    self.walletBalance()
                    let alert = UIAlertController(title: "Message", message: "Wallet Recharged Successfully", preferredStyle: UIAlertControllerStyle.alert)
